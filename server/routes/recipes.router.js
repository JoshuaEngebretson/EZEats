...
router.get("/", rejectUnauthenticated, (req, res) => {
    ...
    */
	const selectAllOfThisUsersRecipes = `
        SELECT 
            recipes.id AS id,
	...
	pool.query(selectAllOfThisUsersRecipes, [userId])
		.then((result) => {
...
router.get(
	...
	(req, res) => {
		...
		pool.query(sqlQuery)
			...
			.catch((dbErr) => {
				console.log(
					"Error inside GET /recipe-categories:",
					dbErr
				);
				res.sendStatus(500);
			});
    ...
    */
	const sqlQuery = `
        ...
        WHERE
            user_id = $1
        ORDER BY
            times_cooked DESC, recipe_name, category.name
        LIMIT 5;
    `;
...
router.get("/shopping-list", rejectUnauthenticated, (req, res) => {
	...
	const shoppingListCardsQuery = `
        ...
        JOIN category
            ON category.id = recipes.category_id
        WHERE
            user_id = $1 AND recipes.on_menu > 0
        ORDER BY
            times_cooked DESC, recipe_name, category.name;
    `;

	pool.query(shoppingListCardsQuery, [userId])
		.then((result) => {
			...
			const shoppingListIngredientsQuery = `
                SELECT
                    JSON_BUILD_OBJECT(
                        ...
                        'multipliedQuantity',
                            (recipe_ingredients.quantity*recipes.on_menu),
                        'conversionCategory',
                            units_of_measurement.conversion_category,
                        'originalUnits',
                            units_of_measurement.unit,
                        'multipliedConvertedQuantity', 
                            (recipe_ingredients.converted_quantity*recipes.on_menu),
                        'foodCategory',
                            food_categories.food_category_name,
                        'recipeId',
                            recipes.id 
                    ) AS ingredients
                FROM recipes
                ...
                JOIN units_of_measurement
                    ON units_of_measurement.id=recipe_ingredients.unit_id
                JOIN ingredients
                    ON ingredients.id=recipe_ingredients.ingredients_id
                JOIN category
                    ON category.id=recipes.category_id
                JOIN food_categories
                    ON food_categories.id = ingredients.food_category_id
                WHERE
                    "user".id = $1 AND recipes.on_menu > 0
                GROUP BY
                    recipes.id, recipes.recipe_name, recipes.image_of_recipe,
                    recipes.recipe_text, ingredients.ingredient_name,
                    recipe_ingredients.quantity, units_of_measurement.id,
                    food_categories.food_category_name, recipe_ingredients.id
                ORDER BY
                    food_categories.food_category_name, ingredients.ingredient_name;
            `;

			pool.query(shoppingListIngredientsQuery, [userId]).then(
				(result) => {
					let unformattedIngredients = result.rows;
					// Combine the unformattedIngredients array and start to format them
					let combinedIngredients =
						unformattedIngredients.reduce(
							(result, item) => {
								const {
									ingredient,
									recipeId,
								...
								if (result[key]) {
									...
									result[key] = {
										...item.ingredients,
										recipeIds: [recipeId],
										ingredientAndConversionCategory:
											key,
									};
					...
					combinedIngredients = Object.values(
						combinedIngredients
					);
					let foodCategories = [];
					...
					combinedIngredients.map((ingredient) => {
						const conversionCategory =
							ingredient.conversionCategory;
						const foodCategory = ingredient.foodCategory;
						const multipliedConvertedQuantity =
							ingredient.multipliedConvertedQuantity;
...
router.get(
	...
	(req, res) => {
		pool.query(
			`SELECT * FROM units_of_measurement ORDER BY conversion_category, unit;`
		)
			.then((result) => {
			...
			.catch((dbErr) => {
				...
				res.sendStatus(500);
				console.log(
					"Error in GET units of measurement route:",
					dbErr
				);
			});
	}
...
router.get("/all-ingredients", rejectUnauthenticated, (req, res) => {
	...
	pool.query(allFoodCategoriesQuery).then((result) => {
		...
		const allIngredientsQuery = `
            ...
            JOIN food_categories
                ON food_categories.id = ingredients.food_category_id;
        `;
		pool.query(allIngredientsQuery)
			.then((result) => {
			...
			.catch((dbErr) => {
				...
				res.sendStatus(500);
				console.log(
					"Error inside GET all-ingredients route:",
					dbErr
				);
			});
	});
...
router.get("/:id", rejectUnauthenticated, (req, res) => {
    ...
    */
        ...
        SELECT 
            ...
            JSON_AGG(
                JSON_BUILD_OBJECT(
                    ...
                    'method',
                        recipe_ingredients.method, 
                    'forWhichPart',
                        recipe_ingredients.for_which_part
	...
	pool.query(sqlText, [userId, recipeID])
		.then((result) => {
...
router.post("/", rejectUnauthenticated, async (req, res) => {
	...
	try {
		...
		ingredients.map((i) => {
			...
			if (i.forWhichPart) {
				newRecipeIngredientQuery = `
                    INSERT INTO recipe_ingredients
                        ...
                        converted_unit, unit_id,
                        ingredients_id, method,
                        recipe_id, for_which_part)
                    VALUES
                        ($1, $2, $3, $4, $5, $6, $7, $8);
                `;
			} else {
				newRecipeIngredientQuery = `
                    INSERT INTO recipe_ingredients
                        (quantity, converted_quantity,
                        converted_unit, unit_id, ingredients_id,
                        method, recipe_id)
                    VALUES
                        ($1, $2, $3, $4, $5, $6, $7);
                `;
			}
			const converted_quantity = convertUnitToSmallest(
				i.quantity,
				i.unit.name,
				i.unit.conversion_category
			);
			let converted_unit;
			...
			console.log("*****");
			console.log(
				`for ingredient with id:${i.ingredient.id} and name:${i.ingredient.name}`
			);
			console.log(
				"newRecipeIngredientQuery:",
				newRecipeIngredientQuery
...
router.post(
	...
	(req, res) => {
        ...
        RETURNING id;
    `;
		pool.query(sqlQuery, [newCategory])
			.then((result) => {
			...
			.catch((dbErr) => {
				...
				res.sendStatus(500);
				console.log(
					"Error in POST new category route:",
					dbErr
				);
			});
	}
...
router.post(
	...
	(req, res) => {
        ...
        INSERT INTO units_of_measurement
            (unit, conversion_category)
        VALUES
            ($1, 'other')
        RETURNING id;
    `;
		pool.query(sqlQuery, [newUnit])
			.then((result) => {
			...
			.catch((dbErr) => {
				res.sendStatus(500);
				console.log(
					"Error in POST new units-of-measurement route:",
					dbErr
				);
			});
	}
...
router.post("/food-categories", rejectUnauthenticated, (req, res) => {
	...
	const sqlQuery = `
        ...
        RETURNING id;
    `;
	pool.query(sqlQuery, [newFoodCategory])
		.then((result) => {
		...
		.catch((dbErr) => {
			res.sendStatus(500);
			console.log(
				"Error in POST new food-categories route:",
				dbErr
			);
		});
});
...
router.post("/ingredients", rejectUnauthenticated, (req, res) => {
	...
	const sqlQuery = `
        INSERT INTO ingredients
            (ingredient_name, food_category_id)
        VALUES
            ($1, $2);
    `;
	pool.query(sqlQuery, [
		newIngredient.ingredientName,
		newIngredient.foodCategory,
	])
		.then((result) => {
			res.sendStatus(201);
		})
...
router.put(
    ...
    `;
		...
		pool.query(sqlQuery, [idToUpdate, userId])
			...
			.catch((dbErr) => {
				console.log(
					"Error inside PUT adjust-on-menu:",
					dbErr
				);
				res.sendStatus(500);
			});
...
router.delete("/:id", rejectUnauthenticated, (req, res) => {
	...
	pool.query(sqlText, sqlValues)
		...
		.catch((dbErr) => {
			...
			res.sendStatus(500);
			console.log(
				`Error in DELETE /${idToDelete} route:`,
				dbErr
			);
		});
