...
export default function EditOrAddRecipePageTemplate(props) {
		...
		if (id && deletedIngredientsArray.length > 0) {
			...
			return (
				<div
					...
					onClick={() =>
						handleSaveRecipe(deletedIngredientsArray)
					}
				>
	...
	const cancelButton = () => {
		if (id) {
			...
			return (
				<div
					className="stacked-buttons cancel"
					onClick={() => history.push(`/view-recipe/${id}`)}
				>
	...
	const [toggleCategoryInput, setToggleCategoryInput] =
		useState(false);
	...
	const [ingredientsInputArray, setIngredientsInputArray] =
		useState([initialIngredientObj]);
	...
	if (
		...
		return (
			<div className="page-margin">
				...
				<br />
				<h3 className="table-title center">
					Enter Ingredients
				</h3>
					...
					<table>
						<thead>
							<tr>
								...
								<th>
									Optional:
									<br /> Prepared Method (e.g.,
									sliced, minced)
								</th>
								<th>
									Optional: <br /> For which part of
									the recipe? (e.g., sauce, garnish)
								</th>
								<th>Delete</th>
							</tr>
						</thead>
						<tbody>
							{ingredientsInputArray.map(
				...
				<div className="bottom-container">
					<div className="textarea-container">
						<textarea
							...
							onChange={(e) =>
								handleRecipeTextChange(e.target.value)
							}
						/>
					</div>
