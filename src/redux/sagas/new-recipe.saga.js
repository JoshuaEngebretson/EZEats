...
function* createNewRecipe({ payload: newRecipe }) {
	try {
		...
		if (isNaN(newRecipe.categoryInput)) {
			const { data: newCategory } = yield axios.post(
				"/api/recipes/recipe-categories",
				{ data: newRecipe.categoryInput }
			);
		...
		for (let ingredient of newRecipe.recipeIngredients) {
			if (isNaN(ingredient.unit.id)) {
				...
				// );
				const { data: newUnit } = yield axios.post(
					"/api/recipes/units-of-measurement",
					{ data: ingredient.unit.unit }
				);
				// console.log("newUnit:", newUnit);
				ingredient.unit.id = newUnit.id;
			}
		}
		// console.log("newRecipe within post creations saga:", {
		// 	...newRecipe,
		// });
...
function* fetchNewRecipe() {
	try {
		const { data: newRecipeId } = yield axios.get(
			"/api/recipes/new-recipe-id"
		);
...
function* createNewIngredient({ payload: newIngredient }) {
	try {
		if (isNaN(newIngredient.foodCategory)) {
			const { data: newFoodCategory } = yield axios.post(
				"/api/recipes/food-categories",
				{ data: newIngredient.foodCategory }
			);
